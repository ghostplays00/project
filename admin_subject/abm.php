...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($bookname_err) && empty($author_err) && empty($description_err) && empty($status_err)) {
        $sql = "INSERT INTO abm (bookname, author, description, status) VALUES (?, ?, ?, ?)";
        if ($stmt = mysqli_prepare($link, $sql)) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                header("location: abm.php"); // Redirect to the same page after successful submission
                exit();
            } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['toggle_status'])) {
    ...
    $sql = "UPDATE abm SET status = ? WHERE id = ?";
    if ($stmt = mysqli_prepare($link, $sql)) {
        mysqli_stmt_bind_param($stmt, "si", $new_status, $book_id);
        if (mysqli_stmt_execute($stmt)) {
            header("location: abm.php"); // Redirect to the same page after successful status toggle
            exit();
        } else {
            echo "Oops! Something went wrong. Please try again later.";
        }
...
if(isset($_POST['user_id'])) {
    ...
    $delete_query = "DELETE FROM abm WHERE id = '$user_id'";
