...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($bookname_err) && empty($author_err) && empty($description_err) && empty($status_err)) {
        $sql = "INSERT INTO humms (bookname, author, description, status) VALUES (?, ?, ?, ?)";
        if ($stmt = mysqli_prepare($link, $sql)) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                header("location: humms.php"); // Redirect to the same page after successful submission
                exit();
            } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['toggle_status'])) {
    ...
    $sql = "UPDATE humms SET status = ? WHERE id = ?";
    if ($stmt = mysqli_prepare($link, $sql)) {
        mysqli_stmt_bind_param($stmt, "si", $new_status, $book_id);
        if (mysqli_stmt_execute($stmt)) {
            header("location: humms.php"); // Redirect to the same page after successful status toggle
            exit();
        } else {
            echo "Oops! Something went wrong. Please try again later.";
        }
...
if(isset($_POST['user_id'])) {
    ...
    $delete_query = "DELETE FROM humms WHERE id = '$user_id'";
            ...
            <div class="card-body">
                    ...
                    <tbody>
                        ...
                        <?php
                                    ...
                                    if(isset($_GET['search']))
                                    {
                                        ...
                                        $query = "SELECT * FROM humms WHERE CONCAT(booknamne, author, description, status) LIKE '%$filtervalues%' ";
                                    } else {
                                        $limit = 50; // Number of entries to show in a page.
                                        // Look for a GET variable page if not found default is 1.        
                                        $page = isset($_GET['page']) ? $_GET['page'] : 1;
                                        $start = ($page-1) * $limit;                                 
                                        $query = "SELECT * FROM humms LIMIT $start, $limit";
                                    }
                                        ...
                                        foreach($query_run as $items)
                                        {
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <form action="humms.php" method="POST">
                ...
                // PHP code for displaying pagination links goes here
                        ...
                        if(!isset($_GET['search'])) {
                            ...
                            $result = mysqli_query($link, "SELECT COUNT(id) AS id FROM humms");
                            ...
                            if ($page < $pages) {
                                echo '<a href="humms.php?page=' . ($page + 1) . '" class="btn btn-primary">Next 50 Books</a>';
                            }
                        }
